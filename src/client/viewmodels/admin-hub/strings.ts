...
export const ADMIN_HUB_STRINGS = {
  ...
  statPeopleDetail: (count: number): string => `${count} cadastradas este mes`,
  statRolesDetail: (count: number): string => `${count} assistentes sociais`,
  ...
  lookupDetailTitle: (tableName: string): string => `Detalhes: ${tableName}`,
  lookupCreateEntry: "+ Novo Valor",
  lookupCardAria: (tableName: string, count: number): string =>
    `Ver ${tableName}, ${count} valores ativos`,
  toggleOnAria: (label: string): string => `Desativar valor ${label}`,
  toggleOffAria: (label: string): string => `Ativar valor ${label}`,
